...
import { ServerConnection, KernelManager, ContentsManager } from '@jupyterlab/services'
...
function main(window: CustomWindow) {
  ...
  function requestKernel(settings: {
    ...
    return _kernel_manager.startNew({ name: kernel_type }).then(kernel => kernel)
  }
  ...
  function getKernel() {
    ...
    return requestBinder(repo, branch, binder_url).then(settings => requestKernel(settings))
  }
  ...
  function renderCell(element: HTMLElement, code: string = '') {
    const outputArea = new OutputArea({
      ...
      rendermime: new RenderMimeRegistry({ initialFactories: getRenderers() })
    })

    const $wrapper = _$('div', class_names.wrapper)
    element.replaceWith($wrapper)

    const $title = _$('h4', class_names.title, 'Try Manim Playground')
  ...
  function execute(outputArea: OutputArea, code: string) {
    ...
    new Promise<IKernelConnection>((resolve, reject) => getKernel().then(resolve).catch(reject))
      .then(kernel => {
        _kernel = kernel
        render(outputArea, code)
      })
...
%manim -qm -v WARNING ${manimClassname}`
  ...
  function initManimBinder({
    ...
    allCells.forEach(cell => renderMakeInteractiveButton(cell as HTMLElement))
  }
...
main(window as unknown as CustomWindow)
